...
TRAINING_CMD="itwinai exec-pipeline --config-path configuration_files --config-name juwels_training strategy=ddp run_id=hython-juwels-runall-ddp num_workers_per_trial=$TOT_GPUS"
sbatch --export=ALL,DIST_MODE="$DIST_MODE",RUN_NAME="$RUN_NAME",TRAINING_CMD="$TRAINING_CMD",PYTHON_VENV="$PYTHON_VENV" \
    --job-name="$RUN_NAME-n$N" \
    --output="logs_slurm/job-$RUN_NAME-$NNODES-n$N.out" \
    --error="logs_slurm/job-$RUN_NAME-$NNODES-n$N.err" \
...
TRAINING_CMD="itwinai exec-pipeline --config-path configuration_files --config-name juwels_training strategy=deepspeed run_id=hython-juwels-runall-deepspeed num_workers_per_trial=$TOT_GPUS"
sbatch --export=ALL,DIST_MODE="$DIST_MODE",RUN_NAME="$RUN_NAME",TRAINING_CMD="$TRAINING_CMD",PYTHON_VENV="$PYTHON_VENV" \
    --job-name="$RUN_NAME-n$N" \
    --output="logs_slurm/job-$RUN_NAME-$NNODES-n$N.out" \
    --error="logs_slurm/job-$RUN_NAME-$NNODES-n$N.err" \
